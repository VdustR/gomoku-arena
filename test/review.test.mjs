...
truthy('thinking time is measured for the first move', review.moves[0].thinkingMs >= 50,
...
check('with the reason for each', third.rejected.map((r) => r.reason), ['occupied', 'bad-point'])
...
check('a side with no metrics of a kind does not invent one', review.sides.white.metrics.input_tokens, undefined)
...
check('the first frame is empty', review.positions[0].every((cell) => cell === 0), true)
