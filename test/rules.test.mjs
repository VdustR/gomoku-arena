...
const board = (stones) => { const b = createBoard(); for (const [x, y, c] of stones) b[idx(x, y)] = c; return b }
const row = (y, xs, c) => xs.map((x) => [x, y, c])

// 1. Free style: five in a row wins.
check('free: black five wins',
  resolveMove(board(row(7, [3,4,5,6], BLACK)), 7, 7, BLACK, 'free').status, 'win')

// 2. Free style: overline also wins (no restriction).
check('free: black overline wins',
  resolveMove(board(row(7, [3,4,5,6,7], BLACK)), 8, 7, BLACK, 'free').status, 'win')

// 3. Renju: black overline is forbidden, not a win.
check('renju: black overline forbidden',
  moveLegality(board(row(7, [3,4,5,6,7], BLACK)), 8, 7, BLACK, 'renju').reason, 'overline')

// 4. Renju: white overline still wins.
check('renju: white overline wins',
  resolveMove(board(row(7, [3,4,5,6,7], WHITE)), 8, 7, WHITE, 'renju').status, 'win')

// 5. Renju: black exact five is legal and wins.
check('renju: black exact five legal',
  moveLegality(board(row(7, [3,4,5,6], BLACK)), 7, 7, BLACK, 'renju').legal, true)
check('renju: black exact five wins',
  resolveMove(board(row(7, [3,4,5,6], BLACK)), 7, 7, BLACK, 'renju').status, 'win')
...
check('renju: white double three legal',
  moveLegality(board([...row(7, [5,6], WHITE), [7,5,WHITE], [7,6,WHITE]]), 7, 7, WHITE, 'renju').legal, true)

// 11. Occupied points are rejected.
check('occupied point rejected',
  moveLegality(board([[7,7,BLACK]]), 7, 7, WHITE, 'free').reason, 'occupied')

// 12. Edge of board does not wrap around.
check('no wrap at right edge',
  resolveMove(board([[11,0,BLACK],[12,0,BLACK],[13,0,BLACK],[14,0,BLACK]]), 10, 0, BLACK, 'free').status, 'win')
check('no wrap across rows',
  resolveMove(board([[13,0,BLACK],[14,0,BLACK],[0,1,BLACK],[1,1,BLACK]]), 2, 1, BLACK, 'free').status, 'playing')
